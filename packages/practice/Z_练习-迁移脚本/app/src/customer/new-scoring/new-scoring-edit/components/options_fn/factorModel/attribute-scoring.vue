...
<template>
    <div class="form-field score-rule">
        ...
        <button class="btn btn-sm btn-theme btns" v-if="writable && fieldEventName().flag" @click="addRule" >+{{'newScoring.addRule' | t}}</button>
        <!-- 得分范围 -->
        <div class="form-field">
          <label class="label">{{'newScoring.scoreUpperLimit' | t}}</label>
           + <span>{{rulesObj.highestScore}}</span>
          <p class="attribute-tip">{{'newScoring.rulesDesc' | t}}</p>
        </div>
        <!-- 因子名称 -->
        <div class="form-field">
          <label class="label">{{'newScoring.mark' | t}}</label>
          <input type="text" class="input" v-model.trim="rulesObj.scoringTitle"  
                  v-title:top.line.warn="errorTitle" :class="{error: errorTitle}" :placeholder="$t('newScoring.enterUpTo100Characters')"
...
<script type="text/babel">
    import { ContactApiV1, ScoringV2 } from '@/api';
    ...
    export default {
        ...
        methods: {
            ...
            valid() {
                let lth = 0;
                if (this.$refs.scoringConditions) {
                    const res = this.$refs.scoringConditions.filter(attr => {
            ...
            peopleCount() {
                if (!this.isCreate) {
                    ...
                    if (this.attribute) {
                        ...
                        this.$toast(this.$t('newScoring.fieldAvailable'), 'warn');
                    }   
            ...
            addRule() {
                if (this.rulesObj.rules.length >= 5) {
                    this.$toast(this.$t('newScoring.addFive'), 'warn');
                } else {
