    ...
    <div class="form-field score-rule">
        ...
        <div class="form-field">
          ...
          <input type="text" class="input" v-model.trim="range.scoringTitle" 
                  ...
                  @focusin="errorTitle = ''" :placeholder="$t('scoring.enterUpTo100Characters')" 
        ...
        <div class="form-field">
          ...
          <span>{{range.scoreFloor}}</span> ~ <span>{{range.scoreCeil}}</span>
          <p class="event-tip">{{'scoring.eventRules' | t}}</p>
        </div>
        <!-- 评分规则 -->
        ...
        <div class="form-field" v-if="fieldEventName().flag">
            ...
            <template v-if="rules.timeType === 0">
                ...
                <input  class="input" 
                        ...
                        :disabled="!writable"
                        style="width: 50px;">
                <span>{{'scoring.days' | t}}</span>
                <i class="icon icon-yiwen1" v-title:right="$t('scoring.dateTipNext') + rules.fromDate + $t('scoring.dateTipLast')" ></i>
            </template>
            <template v-if="rules.timeType === 1">
                ...
                <i class="icon icon-yiwen1" v-title:right="$t('scoring.dateTip')"></i>
            </template>
            <!-- 任意or指定radio -->
            <template v-if="isTD && isCustomization">
                <div class="event-radio">
                    <radio :source="0" v-model="rules.isAppoint" :disabled="!writable">{{'scoring.any' | t}}&nbsp;{{eventName}}</radio>
                    <radio :source="1" v-model="rules.isAppoint" :disabled="!writable">{{'scoring.assigned' | t}}&nbsp;{{eventName}}</radio>
                </div>
                <!-- 指定事件 -->
                <template v-if="rules.isAppoint === 1">
                    <div class="event-title">
                        <p>{{eventName}}&nbsp;{{'scoring.list' | t}}</p>
                    </div>
                    <p class="red-text event-tip" v-if="isWarns">{{'scoring.fieldRequired' | t}}</p>
                    <div class="event-hidden" :class="{'over-hidden': rules.conditions.length > 6}">
                            <div class="event-input" v-for="(item, index) in rules.conditions" :key="index">
                                 ...
                                 <!-- 标签 -->
                                  <template v-if="item.field_name === 'label_id'">
                                      <template v-if="rules.event === 'label_add' || rules.event === 'label_remove'">
                                          <selector :options="tabList" v-model="item.value" search :disabled="!writable" @select="wechatChange($event, item)" @invalid="invalid(item)"></selector>
                                      </template>
                                      <!-- 粉丝标签 -->
                                ...
                                <p class="red-text" v-if="item.isWarn">{{'scoring.fieldRequired' | t}}</p>
                            </div>
                    </div>    
                    <!-- 添加事件     -->
                    <div @click="addEvent()" class="add-event"  v-if="writable">
                        ...
                        <span class="theme-text">{{'scoring.add' | t}}&nbsp;{{eventName}}</span>
                    </div>
                </template>
                <!-- 事件是否计一次分 -->
                <div class="event-check">
                    <checkbox :source="true" v-model="rules.isCardinality" v-if="writable">{{'scoring.each' | t}}&nbsp;{{eventName}}&nbsp;{{'scoring.only' | t}}</checkbox>
                </div>
            </template>
            <!-- 预估人数 -->
            <div class="event-estimate">
                <span>{{'scoring.audience' | t}}：</span><span>{{rules.expectedNumber}}</span>
                <svg aria-hidden="true" class="icon icon-reset" v-if="writable" @click="peopleCount">
                      <use xlink:href="#icon-reset"></use>
                </svg>
                <i class="icon icon-yiwen1" v-title:right="$t('scoring.expectedTip')"></i>
            </div>
        </div>
        <!-- 得分 -->
        <div class="form-field">
            <label class="label">{{'scoring.score' | t}}</label>
            ...
            <input  class="input" 
                    ...
                    @blur="numBlur">
            <span class="score-num">{{'scoring.per' | t}}</span>
            <span v-if="rules.openStatus">{{'scoring.scoreLimt' | t}}</span>
            <span v-else>{{'scoring.deductionLimt' | t}}</span>
            <input  class="input" 
                    type="number" 
                    style="width:50px;"
                    :disabled="!writable" 
...
<script type="text/babel">
    import { JourneyApiV3, QuestionnaireApiV1, WechatApiV1, ContactApiV1, WecomApiV1, ContactApi, ScoringV1 } from '@/api';
    ...
    export default {
        ...
        data() {
            return {
                ...
                errorTitle: '', // 名称错误提示
                timeOptions: [
                    {name: this.$t('scoring.relativeDate'), id: 0},
                    {name: this.$t('scoring.absoluteDate'),id: 1}
                ],
                wechat_public: this.$t('scoring.accounts'),
                email: this.$t('scoring.email'),
                sms: this.$t('scoring.sms'),
                qr_code: this.$t('scoring.qrcode'),
                custom_mess: this.$t('scoring.customer'),
                template_mess: this.$t('scoring.tempalte'),
                form: this.$t('scoring.form'),
                mms: this.$t('scoring.mms'),
                label: this.$t('scoring.tag'),
                wecom: this.$t('scoring.wecom'),
                apply_mess: this.$t('scoring.agent'),
                mass: this.$t('scoring.broadcast'),
        ...
        methods: {
            ...
            addEvent() {
                ...
                if (this.rules.conditions.length < 50) {
                    ...
                    this.$toast(this.$t('scoring.addFifty') + this.eventName, 'warn');
                }
