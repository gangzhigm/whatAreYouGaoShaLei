        ...
        <div class="condition">
            <div class="scoring-box">
                <div ref="classify" :class="rulesObj.conditions.length > 1 ? 'classify border border-left' : 'classify'">
                    ...
                    <template>
                        <div class="choose-box">
                            <div class="choose" v-for="(ite,index) in rulesObj.conditions" :key="index" :class="{'last-selector': index + 1 === rulesObj.conditions.length}"> 
                              ...
                              <template v-if="fieldType === 2">
                                  <input v-if="ite.operation === 'gt' || ite.operation === 'lt' || ite.operation === 'eq' || ite.operation === 'ne'" 
                                         :disabled="!writable" v-model.trim="ite.value" class="input md" type="number" maxlength="20">
                              </template>
                            ...
                            <!-- 添加条件 -->
                            <div @click="addScore(rulesObj)" class="add-score" v-if="writable">
                                <svg aria-hidden="true" class="icon add-icon">
                                    <use xlink:href="#icon-add"></use>
                                </svg>
                                <span class="theme-text">{{'scoring.addCondition' | t}}</span>
                            </div>
                        </div>
                    </template>
                </div>
                ...
                <div class="score">
                    ...
                    <span>{{'scoring.points' | t}}</span>
                </div>
                <!-- 预估人数 -->
                <div class="plan-num">
                  <span>{{rulesObj.expectedNumber}}</span>
                  <i class="icon icon-yiwen1" v-title:top="$t('scoring.expectedTip')"></i>
                </div>
                <!-- 删除 -->
                <div class="delete">
...
<script type="text/babel">
    ...
    export default {
        ...
        methods: {
            ...
            validAttr() {
                let isRule = true;
                this.rulesObj.conditions.forEach(item => {
                    // 非空值验证
            ...
            relativeChange(e, ite) {
                ...
                if (ite.operation === 'exists' || ite.operation === 'missing') {
                    ite.isWarn = false;
                }
            },
            // 添加条件
            addScore(item) {
                if (item.conditions.length >= 3) {
                    this.$toast(this.$t('scoring.addThree'), 'warn');
                } else {
