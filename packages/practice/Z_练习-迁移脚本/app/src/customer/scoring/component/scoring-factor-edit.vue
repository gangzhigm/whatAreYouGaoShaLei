...
<template>
    <!-- 编辑评分因子弹窗 -->
    <modal :title="factorRule.id ? $t('scoring.scoringFactorEdit') : (writable ? $t('scoring.addScoringFactor') :  $t('scoring.factorTemplate'))" @close="$emit('close')" size="lg">
...
<script type="text/babel">
    ...
    export default {
        ...
        methods: {
            ...
            // 属性事件确定校验
            attributeOk() {
                this.$refs.attribute.checkField();
                if (this.$refs.attribute.checkField()) {
                    this.$refs.attribute.checkTitle();
                    this.$refs.attribute.valid();
                    if (this.$refs.attribute.valid() === 0 
                        && this.$refs.attribute.checkTitle() 
                        && this.$refs.attribute.checkField()) {
                        this.saveRule();
                    }
            ...
            eventOk() {  
                ...
                if (this.$refs.event.checkField()) {
                    this.$refs.event.checkTitle();
                    this.$refs.event.valid();
                    if (this.eventrRules.isAppoint === 0) {
                        if (this.$refs.event.checkField() && this.$refs.event.checkTitle()) {
                            this.saveRule();
                        }
                    } else {
                        if (this.$refs.event.valid() && this.$refs.event.checkField() && this.$refs.event.checkTitle() ) {
                            this.saveRule();
                        }
                    }
                }
