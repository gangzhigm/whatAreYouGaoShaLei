 ...
 */
export function getLineChart(leftCount, rightCount, XAxisData, series, legendData, percentIndices = []) {
    let yAxis = getYAxis('', leftCount, rightCount);

    XAxisData = XAxisData && XAxisData.length ? XAxisData : [t('eventAnalysis.total')];
    return {
        color: colors,
        tooltip: {
            trigger: 'axis',
