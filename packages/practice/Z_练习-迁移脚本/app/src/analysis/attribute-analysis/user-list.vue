                    ...
                    <div class="inline-user-group-form" v-if="selectShow">
                        ...
                        <div class="inline-form-content">
                            ...
                            <div class="form-field tar">
                                <button class="btn btn-md btn-white" @click="hideSelect">
                                    {{'common.cancel' | t}}
                                </button>
                                <button class="btn btn-md btn-theme" @click="saveGroup" :disabled="!valid">
                                    {{'common.confirm' | t}}
                                </button>
                            </div>
                        </div>
            ...
            <div class="table-wrap table-user-list" v-if="userList.length && showList.length">
                <div class="table-box" :class="{scroll:showList.length > 3}">
                    <table class="table">
                        <thead>
                        <tr>
                            <th v-for="(user,index) in userAttrList" @click="sortList(user)"
                        ...
                        <tbody>
                        <tr v-for="(user,index) in userList" @click="routerEvent(user[0])">
        ...
        computed: {
            ...
            crumbs() {
                return [
                    ...
                    this.data.bookmarkId
                        ? {
                            name: 'attribute',
                            text: this.$t('attributeAnalysis.attributeAnalysis'),
                            query: {bmsId: this.data.bookmarkId}
                        }
                        : {name: 'attribute', text: this.$t('attributeAnalysis.attributeAnalysis')},
                    {
            ...
            groupTitle() {
                ...
                let bookmarkName = this.titleData.bookmarkName;
                let title = '';
                ...
                str.forEach((item, idx) => {
                    title += nameList[idx] ? `${nameList[idx]}：${item}` : item;
                    if (idx + 1 < str.length) {
                        title += '，';
                    }
                });
        ...
        methods: {
            ...
            downUserList() {
                ...
                downCsv(
                    ...
                    'param',
                    JSON.stringify(data),
                    LINKS.QUERY_API_BASE + '/api/export/user/analytics/list'
                );
            }
        }
    };
