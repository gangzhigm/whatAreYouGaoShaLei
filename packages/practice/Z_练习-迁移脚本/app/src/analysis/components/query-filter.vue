<template>
    <div class="event-analysis-filter" :class="{indent, 'only-one': filter.conditions.length === 1}"
        ...
        <condition v-for="(condition, index) in conditions" :groups="groups" :condition="condition"
                   :incompleteAsValid="incompleteAsValid" :autoValidation="autoValidation"
                   :eventIdList="eventIdList" :evaluateValueLists="evaluateValueLists"
                   :key="condition.id" @change="modifyCondition($event, index)"></condition>
    </div>
</template>
<script>
    // filters editor
    import Condition, {isCompleteCondition} from './query-filter-condition.vue';
