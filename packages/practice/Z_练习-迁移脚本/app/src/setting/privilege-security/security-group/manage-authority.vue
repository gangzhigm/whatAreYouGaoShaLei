<template>
    <div class="manage-authority">
        <p class="operate-text">{{'privilege.settingManage' | t}}</p>
        <div class="content-table">
            ...
            <ul v-for="(item,index) in value" class="row table-content">
                ...
                <ul v-for="(itm,idx) in item.menuList" class="row-level-two" :class="{margin:idx!==0}">
                    <!-- 页面 -->
                    <li class="level-two">
                        <checkbox v-model="itm.showType" @input="changePage(itm,item,idx)">{{itm.menuName}}</checkbox>
                    </li>
                    <div class="row-level-three" :class="{marginThree:idx!==0}">
                        <!-- 基础权限 -->
                        <p v-for="(itms,i) in itm.menuList">
                            <checkbox v-model="itms.showType" @input="changeFunction(itms,item,idx)">{{itms.menuName}}
                            </checkbox>
                        </p>
                        <p v-if="itm.menuList.length <= 0">-</p>
                    </div>
                </ul>
            </ul>
            <ul class="row table-content">
                <list-empty v-if="value.length===0"/>
            </ul>
        </div>
    </div>
</template>
                    ...
                    if (itms.showType) {
                        ...
                        item.menuList.forEach((menu, index) => {
                            menu.menuList.forEach(itm => {
                                if (itm.showType !== itms.showType && index !== 0) {
                                    selectAll = false;
                                }
                            });
                        });
