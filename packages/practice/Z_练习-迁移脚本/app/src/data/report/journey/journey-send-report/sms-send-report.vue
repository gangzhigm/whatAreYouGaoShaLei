        ...
        <div class="page-content">
            ...
            <div class="content-view margin report-header">
                ...
                <task-selector
                    ...
                    @search="searchTask"
                    @load="loadMoreTask"
                    @checkOptions="changeTaskId"
                    @confirm="confirmTask"></task-selector>
            ...
            <div class="content-view margin">
                <div class="charts">
                    ...
                    <div class="chart half right-margin">
                        ...
                        <div :class="['chart-body','md','table', {empty: !sendReport && !sendTotal_loading}]">
                            <template v-if="sendReport && !sendTotal_loading">
                                <table>
                                    <thead>
                                    <tr>
                    ...
                    <div class="chart half">
                        ...
                        <div :class="['md', 'chart-body', {empty: !sendReport && !charts.smsChart_loading}]" ref="smsChart"></div>
                    </div>
                </div>
                ...
                <div class="charts">
                    ...
                    <div class="chart one-third right-margin">
                        ...
                        <div :class="['chart-body','md', {empty: !reportMap.servicesProportion && !charts.operatorChart_loading}]" ref="operatorChart"></div>
                    </div>
                        <!--2.报错统计-->
                    <div class="chart one-third right-margin">
                        <div class="chart-head">
                        ...
                        <div :class="['chart-body','md','table', {empty: !reportMap.errorCountMap && !errorTotal_loading}]">
                            <template v-if="reportMap.errorCountMap && !errorTotal_loading">
                                <table>
                                    <thead>
                                    <tr>
                                        <th>{{ $t('data.statisticalItem') }}</th>
                            ...
                            <ring-loading v-if="errorTotal_loading"/>
                        </div>
                    </div>
                    <!--3. 错误统计-->
                    <div class="chart one-third">
                        ...
                        <div :class="['chart-body', 'md', {empty: !reportMap.errorCountMap && !charts.errorChart_loading}]" ref="errorChart"></div>
                    </div>
                </div>
                <!--发送历史-->
                <div class="charts">
    ...
    export default {
        data() {
            return {
                ...
                charts: {
                    smsChart: null,
                    smsChart_loading: true,
                    errorChart: null,
                    errorChart_loading: true,
                    operatorChart: null,
                    operatorChart_loading: true,
                },
                smsDate: [],
                errorDate: [],
        ...
        computed: {
            ...
            query() {
                ...
                if (this.dataSourceId === 3 || this.dataSourceId === 2) {
                    ...
                    let list = [];
                    this.originTaskList.map(item => {
                        if (this.taskIdList.indexOf(item.id) !== -1) {
                            list.push(item.campaignId);
                        }
                    });
            ...
            sendData(){
                return {
                    ...
                    type: this.TYPE ? MMSTYPE : SMSTYPE
                };
            }
        },
        watch: {
        ...
        methods: {
            ...
            checkerTask(taskList = [], id) {
                ...
                if (vm.totalPageTask >= vm.taskPageNumber && targetTask.length === 0) {
            ...
            loadMoreTask(checkerTask) {
                ...
                // 0:邮件,1:短信,2:微信,3:彩信
                    .getTasks([], '', [this.journeyId], this.taskPageNumber++, this.taskKeyWord, '', this.TYPE ? MMSTYPE : SMSTYPE,
                        this.allChecked === true ? null : this.taskIdList, this.pageSize)
            ...
            getOperatorList() {
                let ids = '';
                //chart5
                this.charts.operatorChart_loading = true;
                this.charts.operatorChart = getChartInstance(this.$refs.operatorChart);
                showLoading(this.charts.operatorChart);
            ...
            searchTask(searchResult) {
                this.taskKeyWord = searchResult.trim();
                // this.taskList = [];
            ...
            smsCharts() {
                ...
                this.$nextTick(() => {
                    ...
                    this.charts.smsChart_loading = false;
                });
            },
            // chart4 chart5
            // 错误统计图表
            errorCharts() {
                ...
                this.$nextTick(() => {
                    ...
                    hideLoading(this.charts.errorChart);
                    this.charts.errorChart_loading = false;
                });
            },
            // chart3
            // 运营商分布
            operatorCharts() {
                ...
                this.$nextTick(() => {
                    ...
                    hideLoading(this.charts.operatorChart);
                    this.charts.operatorChart_loading = false;
                });
            },
            goToSmsReport() {
                this.$router.push({
            ...
            changeTaskId(){
                ...
                let taskDetail = this.taskList.filter((task)=>{
                    return this.taskIdList.indexOf(task.id) !== -1;
                });
                let ids = taskDetail.map((item)=>{
                    return item.campaignId;
                });
                if (this.totalRow > this.pageSize) {
                    this.pageSize = this.totalRow;
                }
                this.campaignId = this.allChecked ? -1 : ids;
                // this.taskList = [];
                this.taskPageNumber = 1;
