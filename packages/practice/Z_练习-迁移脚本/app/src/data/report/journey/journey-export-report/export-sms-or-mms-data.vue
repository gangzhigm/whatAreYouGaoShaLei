<template>
    ...
    <div class="filter-view">
            ...
            <div class="field-group">
                <div class="click-box animation" @click="listOpen">
                    ...
                    <span v-if="chosenMissionIds.length === 1"
                          :title="$t('data.chosen') + selectTaskName + $t('data.se')"
                          v-text="$t('data.chosen') + selectTaskName + $t('data.se')"></span>
                    <span v-if="chosenMissionIds.length > 1"
        ...
        <div class="form-field flex">
            ...
            <div class="field-group">
                <!-- 导出短信原始数据 -->
                <button v-show="dataType === 'SMS'"
                        :disabled="$has('datacentre_marketing_export_sms')"
                        class="btn btn-theme btn-md" @click="exportReport">{{'data.export' | t}}
                </button>
                <!-- 导出彩信原始数据 -->
                <button v-show="dataType === 'MMS'"
                        :disabled="$has('datacentre_marketing_export_marketing')"
                        class="btn btn-theme btn-md" @click="exportReport">{{'data.export' | t}}
                </button>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.$nextTick(() => {
                if (this.journeyData) {
                    ...
                    if (this.journal && this.journeyData.taskId.length !== 0) {
        ...
        methods: {
            ...
            getTask(checkPageNo, mode) {
                ...
                JourneyApiV2
                    ...
                    .then(res => {
                        ...
                        this.totalTaskList.$key = 'campaignId';
                        // 其他模块进入携带的导出数据源
                        if (this.journeyData.taskId && !this.missionIds.length &&
                            !this.chosenMissionIds.length && !this.missionName) {
                            this.missionList.$key = 'campaignId';
                            // 其他模块传的是id，这里需要转换campaignId
                            // let curIndex = this.missionList.findIndex(list => list.id === this.journeyData.taskId);
                            let curIndex = [];
                            // let campaignIds = [];
                            if (typeof (this.journeyData.taskId) !== 'object') {
                                this.journeyData.taskId = [this.journeyData.taskId];
                            }
                            if (this.journeyData.taskId !== -1) { //非全选
                                this.missionList.filter((item,index)=>{
                                    if (this.journeyData.taskId.indexOf(item.id) !== -1) {
                                        this.campaignIds.push(item.campaignId);
                                        curIndex.push(index);
                                    }
                                });
                            }
                            if (this.campaignIds.length !== this.journeyData.taskId.length) { //判断回显的数据和所选的数据是否相同
                                if (this.missionTotalPage > this.missionPageNo) {
                                    this.missionPageNo += 1;
                                    this.getTask();
                                }
                            } else {
                                this.missionIds = this.campaignIds;
                                this.chosenMissionIds = this.campaignIds;
            ...
            // 导出
            exportReport() {
                if ((!this.exportData && this.exportData !== 0) || !this.journal || !this.chosenMissionIds.length
                    || !this.exportEmail) {
            ...
            judgeSubject() {
                if (this.dataType === 'SMS') {
                    this.curEmailSubject = this.$t('data.smsData');
                } else {
                    this.curEmailSubject = this.$t('data.mmsData');
                }
            },
            clear() {
                this.journalType = 0;
