...
import DATA_MODEL from '../lib/model.json';
import DATA_SYMBOL from '../lib/symbol.json';
import NODE_CHILDPROCESS from 'child_process';
import DATA_CHAPTERS from '../lib/chapters.json';
import DATA_GITGNORE from '../lib/gitgnore.json';
const SYMBOL = DATA_SYMBOL.symbol;
const MODEL_LIST = DATA_MODEL.model;
const PLUGIN_LIST = DATA_MODEL.plugin;
const GITGNORE = Object.keys(DATA_GITGNORE);
const SHORT_SENTENCE = DATA_SYMBOL.shortSentence;
const OBJ = {
    path: './',
    name: '',
    deep: 1,
...
const TEXT = [
    ...
    DATA_CHAPTERS['nine'],
    DATA_CHAPTERS['ten'],
    DATA_CHAPTERS['eleven'],
    DATA_CHAPTERS['twelve'],
    DATA_CHAPTERS['thirteen']
];
...
function displayFileOne() {
    // 1.赋值项目名
    TEXT[0][0] += NODE_PATH.resolve(SYMBOL[0]).substring(
        NODE_PATH.resolve(SYMBOL[0]).lastIndexOf(SYMBOL[7]) + 1,
        NODE_PATH.resolve(SYMBOL[0]).length
    );
    ...
    for (let [index, item] of chapterArray.entries()) {
        ...
        TEXT[0].splice(
            TEXT[0].length - 1,
            0,
            `${SYMBOL[8]}${chapterArray[index][2].substring(
                chapterArray[index][2].indexOf(SYMBOL[2]) + SYMBOL[2].length,
                chapterArray[index][2].lastIndexOf(SYMBOL[2])
            )}${SYMBOL[9]}${chapterArray[index][1].substring(
                chapterArray[index][1].lastIndexOf(SYMBOL[1]) + SYMBOL[1].length,
                chapterArray[index][1].length
            )}${SYMBOL[10]}`
        );
        TEXT[0].splice(TEXT[0].length - 1, 0, `${SYMBOL[6]}`);
    }
    ...
    for (let i of Object.keys(packageDevdependencies)) {
        if (MODEL_LIST.find((item) => item[0] === i)) {
            const VERSION =
                packageDevdependencies[i].indexOf('^') === 0
                    ? packageDevdependencies[i].substring(1)
                    : packageDevdependencies[i];
            TEXT[3].push(
                `${SYMBOL[11]}${i}${SYMBOL[12]}${VERSION}${SYMBOL[13]}${
                    MODEL_LIST.find((item) => item[0] === i)[1]
                }`
            );
            TEXT[3].push(SYMBOL[6]);
        }
    ...
    // 4.赋值分支
    const BRANCH = NODE_CHILDPROCESS.execSync(SHORT_SENTENCE[0])
        .toString()
        .split('\n')
        .filter((item) => item.includes('remotes/origin/') && !item.includes(' -> '))
        .map((item) =>
            item.substring(item.lastIndexOf(SYMBOL[15]) + SYMBOL[15].length, item.length)
        );
    for (let i of BRANCH) {
    ...
    for (let i of Object.keys(packageDevdependencies)) {
        if (PLUGIN_LIST.find((item) => item[0] === i)) {
            ...
            TEXT[10].push(
                `${SYMBOL[11]}${i}${SYMBOL[12]}${VERSION}${SYMBOL[13]}${
                    PLUGIN_LIST.find((item) => item[0] === i)[1]
                }`
            );
            TEXT[10].push(SYMBOL[6]);
        }
...
function writeFile() {
    ...
    for (let i of TEXT) {
        for (let j of i) {
            NODE_FS.appendFileSync(SHORT_SENTENCE[3], `${j}${SYMBOL[14]}`, SHORT_SENTENCE[4]);
        }
    }
}
function displayFile(objTree) {
    // 生成绝对路径
    const PARAMS =
        objTree.path.indexOf(SYMBOL[0]) === 0 ? NODE_PATH.resolve(objTree.path) : objTree.path;
    // 循环检索路径
    NODE_FS.stat(PARAMS, function (err, stats) {
        ...
        if (
            GITGNORE.filter(
                (item) =>
                    PARAMS === `${originPath}${SYMBOL[7]}${item}` ||
                    PARAMS.indexOf(`${originPath}${SYMBOL[7]}${item}${SYMBOL[7]}`) === 0
            ).length === 0
        ) {
            if (OBJ.name === '') {
                originPath = PARAMS;
            ...
            let objFileTree = '';
            if (objTree.deep === 1) {
                objFileTree = `${PARAMS.substring(
                    PARAMS.lastIndexOf(SYMBOL[7]) + 1,
                    PARAMS.length
                )}`;
            } else if (objTree.isLast && objTree.isLast === true) {
                objFileTree = `${SYMBOL[5]}${PARAMS.substring(
                    PARAMS.lastIndexOf(SYMBOL[7]) + 1,
                    PARAMS.length
                )}`;
            } else {
                objFileTree = `${SYMBOL[4]}${PARAMS.substring(
                    PARAMS.lastIndexOf(SYMBOL[7]) + 1,
                    PARAMS.length
                )}`;
            }
            ...
            if (stats.isDirectory()) {
                ...
                for (let i = 0; i < ITEM_DIR_ARRAY.length; i++) {
                    const absolutePath = NODE_PATH.resolve(
                        NODE_PATH.join(PARAMS, ITEM_DIR_ARRAY[i])
                    );

                    if (
                        (NODE_FS.statSync(absolutePath).isDirectory() &&
                            GITGNORE.filter((item) =>
                                absolutePath.includes(`${originPath}${SYMBOL[7]}${item}`)
                            ).length === 0) ||
                        !NODE_FS.statSync(absolutePath).isDirectory()
                    ) {
                        items.children.push({
                            ...
                            name: absolutePath.substring(
                                absolutePath.lastIndexOf(SYMBOL[7]) + 1,
                                absolutePath.length
                            ),
                            type: NODE_FS.statSync(absolutePath).isDirectory()
                                ? SHORT_SENTENCE[5]
                                : SHORT_SENTENCE[6],
                            children: []
                        });
...
function findModel() {
    ...
    if (
        NODE_FS.readdirSync('./').includes('package.json') &&
        JSON.parse(NODE_FS.readFileSync('./package.json', 'utf-8')).devDependencies &&
        JSON.parse(NODE_FS.readFileSync('./package.json', 'utf-8')).dependencies
    ) {
        packageDevdependencies = Object.assign(
            JSON.parse(NODE_FS.readFileSync('./package.json', 'utf-8')).devDependencies,
            JSON.parse(NODE_FS.readFileSync('./package.json', 'utf-8')).dependencies
        );
    } else if (
        ...
        packageDevdependencies = JSON.parse(NODE_FS.readFileSync('./package.json', 'utf-8'))
            .devDependencies;
    } else if (
        NODE_FS.readdirSync('./').includes('package.json') &&
        !JSON.parse(NODE_FS.readFileSync('./package.json', 'utf-8')).devDependencies &&
...
export { SYMBOL, SHORT_SENTENCE, MODEL_LIST, PLUGIN_LIST, GITGNORE, TEXT };
