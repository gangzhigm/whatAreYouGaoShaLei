...
function readData(dataFile) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const fileStream = fs_1.default.createReadStream(dataFile);
        ...
        const inputTokenIndex = inputCharacterList.reduce((prev, curr, idx) => (prev[curr] = idx, prev), {});
        const targetTokenIndex = targetCharacterList.reduce((prev, curr, idx) => (prev[curr] = idx, prev), {});
        // Save the token indices to file.
        const metadataJsonPath = path_1.default.join(args.artifacts_dir, 'metadata.json');
        if (!fs_1.default.existsSync(path_1.default.dirname(metadataJsonPath))) {
            mkdirp_1.default.sync(path_1.default.dirname(metadataJsonPath));
        }
        const metadata = {
        ...
        fs_1.default.writeFileSync(metadataJsonPath, JSON.stringify(metadata));
        ...
        for (const [i, [inputText, targetText]] of zip(inputTexts, targetTexts).entries()) {
...
Returns:
  ...
  model: `keras.Model` instance, the entire translation model that can be
    used in training.
*/
function seq2seqModel(numEncoderTokens, numDecoderTokens, latentDim) {
    // Define an input sequence and process it.
    const encoderInputs = tf.layers.input({
        shape: [null, numEncoderTokens],
        name: 'encoderInputs',
    });
    ...
    const [decoderOutputs,] = decoderLstm.apply([decoderInputs, ...encoderStates]);
...
"""
*/
function decodeSequence(inputSeq, encoderModel, decoderModel, numDecoderTokens, targetBeginIndex, reverseTargetCharIndex, maxDecoderSeqLength) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let decodedSentence = '';
        while (!stopCondition) {
            const [outputTokens, h, c] = decoderModel.predict([targetSeq.toTensor(), ...statesValue]);
            // Sample a token
            const sampledTokenIndex = yield outputTokens.squeeze().argMax(-1).array();
            ...
            targetSeq = tf.buffer([1, 1, numDecoderTokens], 'float32');
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const { inputTexts, maxDecoderSeqLength, numEncoderTokens, numDecoderTokens, targetTokenIndex, encoderInputData, decoderInputData, decoderTargetData, } = yield readData(args.data_path);
        const { encoderInputs, encoderStates, decoderInputs, decoderLstm, decoderDense, model, } = seq2seqModel(numEncoderTokens, numDecoderTokens, args.latent_dim);
        // Run training.
        model.compile({
            optimizer: 'rmsprop',
        ...
        model.summary();
        if (args.logDir != null) {
            console.log(`To view logs in tensorboard, do:\n` +
                `  tensorboard --logdir ${args.logDir}\n`);
        }
        yield model.fit([encoderInputData, decoderInputData], decoderTargetData, {
        ...
        yield model.save(`file://${args.artifacts_dir}`);
        ...
        let [decoderOutputs, stateH, stateC] = decoderLstm.apply([decoderInputs, ...decoderStatesInputs]);
        ...
        // something readable.
        const reverseTargetCharIndex = invertKv(targetTokenIndex);
        ...
        for (let seqIndex = 0; seqIndex < args.num_test_sentences; seqIndex++) {
            ...
            // Get expected output
            const targetSeqVoc = decoderTargetData.slice(seqIndex, 1).squeeze([0]);
            const targetSeqTensor = targetSeqVoc.argMax(-1);
            const targetSeqList = yield targetSeqTensor.array();
            // One-hot to index
            const targetSeq = targetSeqList.map(indice => reverseTargetCharIndex[indice]);
            // Array to string
            const targetSeqStr = targetSeq.join('').replace('\n', '');
            const decodedSentence = yield decodeSequence(inputSeq, encoderModel, decoderModel, numDecoderTokens, targetBeginIndex, reverseTargetCharIndex, maxDecoderSeqLength);
            console.log('-');
